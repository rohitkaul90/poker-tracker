...
class ReadsService {
  ...
  Future<List<PlayerReadNote>> fetchNotes(String readId) async {
  ...
  Future<void> updateRead(String readId, {String? playerLabel, List<String>? tags}) async {
    final updates = <String, dynamic>{'updated_at': DateTime.now().toIso8601String()};
    if (playerLabel != null) { updates['player_label'] = playerLabel; }
    if (tags != null) { updates['tags'] = tags; }
    await _client.from('player_reads').update(updates).eq('id', readId);
  }
  ...
  Future<void> updateNote(String noteId, {
    ...
    await _client.from('player_read_notes').update({
  ...
  Future<void> deleteNote(String noteId) async {
    await _client.from('player_read_notes').delete().eq('id', noteId);
  }

  Future<void> deleteRead(String readId) async {
    await _client.from('player_reads').delete().eq('id', readId);
  }
}
